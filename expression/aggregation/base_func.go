...
func (a *baseFuncDesc) typeInfer4Sum(ctx sessionctx.Context) {
	...
	case mysql.TypeTiny, mysql.TypeShort, mysql.TypeInt24, mysql.TypeLong, mysql.TypeLonglong:
		...
		logutil.Logger(context.Background()).Warn("set field type Flen to MaxDecimalWidth", zap.String("address", fmt.Sprintf("%p", a.RetTp)), zap.String("sql", ctx.GetSessionVars().StmtCtx.OriginalSQL), zap.Stack("stack"))
